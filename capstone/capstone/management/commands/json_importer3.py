...
def log_changes(pub_instance, new_stars, new_open, log_file):
    ...
    if (old_stars == 3 or new_stars == 3) and old_open != new_open:
...
def calculate_json_stats(fp):
    ...
    for p_data in pubs:
        ...
        if not isinstance(stars_val, int):
            print(f"  Warning in JSON stats: 'Inventory Stars' for {p_data.get('Pub Name','N/A')} is not int: '{stars_val}'. Defaulting to 0.")
            stars_val = 0
        
        is_open_val = p_data.get("Open", False)
        if not isinstance(is_open_val, bool):
            print(f"  Warning in JSON stats: 'Open' for {p_data.get('Pub Name','N/A')} is not bool: '{is_open_val}'. Defaulting to False.")
            is_open_val = False
...
def handle_exact_match(pub_instance, data_from_json, log_file):
    new_values = {
    ...
    for attr, new_val in new_values.items():
        ...
        new_val_casted = new_val # Assume types from normalized_pubs_data are correct for direct assignment
    ...
    if dirty_fields:
        if 'camra_id' in dirty_fields and new_values['camra_id']:
            if Pub.objects.filter(camra_id=new_values['camra_id']).exclude(pk=pub_instance.pk).exists():
                log_file.write(f"  ERROR: CAMRA ID {new_values['camra_id']} already exists for another pub. Skipping update for {pub_instance.name}.\n")
...
class Command(BaseCommand):
    help = "Import data from multi-region scraper JSON (cleaner format) into Pub model. Uses /tmp for logs on App Engine."
    ...
    def handle(self, *args, **options):
        ...
        for p_raw in raw_pubs_from_file:
            ...
            if camra_id_val and str(camra_id_val).strip():
                ...
                if url_for_id_extraction:
                    final_camra_id = extract_camra_id(url_for_id_extraction)
            
            stars_val = p_raw.get("Inventory Stars", 0)
            if not isinstance(stars_val, int):
                try:
                    stars_val = int(str(stars_val).strip()) if str(stars_val).strip().isdigit() else 0
                except ValueError: # Catch if strip or isdigit results in something uncastable
            ...
            if not isinstance(open_val, bool):
                open_val = str(open_val).lower() == 'true' # Simple conversion for non-bools
            ...
            if lat_val_str:
                try: 
                    latitude_float = float(lat_val_str)
                except ValueError: 
                    self.stdout.write(self.style.WARNING(f"  Warning: Could not convert latitude '{lat_val_str}' to float for {current_pub_name_for_log}"))
            
            longitude_float = None
            if lon_val_str:
                try: 
                    longitude_float = float(lon_val_str)
                except ValueError: 
                    self.stdout.write(self.style.WARNING(f"  Warning: Could not convert longitude '{lon_val_str}' to float for {current_pub_name_for_log}"))

            normalized_pubs_data.append({
                "Pub Name":        p_raw.get("Pub Name", ""),
                "Address":         p_raw.get("Address", ""),
                "Description":     p_raw.get("Description", ""),
                "Inventory Stars": stars_val,
                "Listed":          p_raw.get("Listed", ""),
                "Open":            open_val,
                "Url":             p_raw.get("Url", ""),
                "Camra ID":        final_camra_id,
        ...
        with open(log_path, "w", encoding='utf-8') as log:
            ...
            if mode == "fresh_import":
                ...
                self.stdout.write(f"✅ Update mode complete. Pubs created: {pubs_created_count}, Pubs skipped by user: {pubs_skipped_by_user_count}.")
                log.write(f"\nUpdate mode complete. Pubs created: {pubs_created_count}, Pubs skipped by user: {pubs_skipped_by_user_count}.\n")
